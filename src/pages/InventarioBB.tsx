...
export default function InventarioBB() {
  ...
  const [formData, setFormData] = useState<{
    ...
    type: 'INGRESO',
    notes: '',
    items: [{ ubicacion: 'Incubadora 01', huevosIngresados: 0, pollosNacidos: 0, enviadosLaboratorio: 0, precioCosto: 0, cantidad: 0 }]
  });

  useEffect(() => {
    const q = query(
      collection(db, 'inventoryLogs'),
      where('animalType', '==', 'POLLO_BB'),
      orderBy('date', 'desc')
    );
  ...
  const handleAddItem = () => {
    setFormData({
      ...formData,
      items: [...formData.items, { ubicacion: 'Incubadora 01', huevosIngresados: 0, pollosNacidos: 0, enviadosLaboratorio: 0, precioCosto: 0, cantidad: 0 }]
    });
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.type === 'INGRESO') {
      totalQuantity = formData.items.reduce((acc, item) => acc + ((item.pollosNacidos || 0) - (item.enviadosLaboratorio || 0)), 0);
    } else {
    ...
    try {
      await addDoc(collection(db, 'inventoryLogs'), {
        type: formData.type, // 'MORTALIDAD' will deduct structurally in Reportes
        animalType: 'POLLO_BB',
        date: Date.now(),
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-bold text-white">Inventario de Pollos BB</h1>
          <p className="text-slate-400">Gestión de nacimientos en planta de incubación.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white/5 backdrop-blur-md border border-white/10 p-6 rounded-3xl flex items-center space-x-4">
          ...
          <div>
            <p className="text-sm font-medium text-slate-400 uppercase tracking-wider">Total Histórico Nacidos</p>
        ...
        <table className="min-w-full divide-y divide-white/10 text-sm">
          <thead className="bg-white/5">
            <tr>
              ...
              <th className="px-6 py-4 text-left text-xs font-semibold text-indigo-300 uppercase tracking-wider">Detalle de Aves</th>
                ...
                <td className="px-6 py-4 text-slate-300 text-xs">
                    {log.items?.map((item, idx) => (
                        <div key={idx} className="mb-1">
                            <span className={`font-bold ${log.type === 'INGRESO' ? 'text-indigo-300' : 'text-red-300'}`}>{item.ubicacion}:</span>{' '}
                            {log.type === 'INGRESO' ? (
                                `HI: ${item.huevosIngresados} | Nacidos: ${item.pollosNacidos} | Lab: ${item.enviadosLaboratorio} | Costo Unit: S/ ${item.precioCosto}`
                            ) : (
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/90 backdrop-blur-sm flex items-start justify-center p-4 overflow-y-auto pt-10 pb-20">
          <div className="bg-slate-900/90 backdrop-blur-xl border border-white/10 rounded-3xl shadow-2xl w-full max-w-4xl p-6 relative">
            <h2 className="text-xl font-bold text-white mb-6">
                {formData.type === 'INGRESO' ? 'Nuevo Ingreso (Nacimientos)' : 'Informar Mortalidad (Bajas)'} - Pollos BB
            </h2>
              ...
              <div className="space-y-4">
                ...
                {formData.items.map((item, idx) => (
                    <div key={idx} className="bg-white/5 border border-white/10 rounded-2xl p-4 flex flex-wrap gap-4 items-end relative">
                        ...
                        <div className="w-64">
                            <label className="block text-xs font-medium text-slate-400 mb-1">Incubadora</label>
                            <select 
                            ...
                            >
                                {['Incubadora 01', 'Incubadora 02', 'Incubadora 03', 'Incubadora 04', 'Incubadora 05', 'Incubadora 06'].map(inc => (
                        ...
                        {formData.type === 'INGRESO' ? (
                            <>
                                <div className="flex-1 min-w-[120px]">
                                    <label className="block text-xs font-medium text-slate-400 mb-1">Huevos Ingresados (HI)</label>
                                    <input 
                                    ...
                                    value={item.huevosIngresados || ''}
                                    onChange={e => handleItemChange(idx, 'huevosIngresados', Number(e.target.value))}
                                    />
                                </div>
                                <div className="flex-1 min-w-[120px]">
                                    <label className="block text-xs font-medium text-slate-400 mb-1">Pollos Nacidos</label>
                                    <input 
                                    type="number" min="0" required
                                    ...
                                    value={item.pollosNacidos || ''}
                                    onChange={e => handleItemChange(idx, 'pollosNacidos', Number(e.target.value))}
                                    />
                                </div>
                                <div className="flex-1 min-w-[120px]">
                                    <label className="block text-xs font-medium text-slate-400 mb-1">Envio Lab.</label>
                                    <input 
                                    type="number" min="0" required
                                    ...
                                    value={item.enviadosLaboratorio || ''}
                                    onChange={e => handleItemChange(idx, 'enviadosLaboratorio', Number(e.target.value))}
                                    />
                                </div>
                                <div className="flex-1 min-w-[120px]">
